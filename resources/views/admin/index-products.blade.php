                ...
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane tab-pane-admin active">
                        <div class="form-group form-inline">
                            ...
                            <a href="{{ route('product-barcodes',['print'=>'1']) }}" class="btn btn-warning" target="_blank">
                                <span class="fa fa-barcode" aria-hidden="true"></span>&nbsp; Print Barcodes
                            </a>
                        </div>
...
<script type="text/javascript">
    ...
    function product_toggle_featured(id) {
        $.get('{{ route("product-toggle-featured") }}', {
            id: id
        }, function(data) {
    ...
    function product_toggle_active(id) {
        $.get('{{ route("product-toggle-active") }}', {
            id: id
        }, function(data) {
    ...
    function attribute_delete(id, name, option, element) {
        if (confirm('Are you sure you want to remove ' + name + ': ' + option)) {
            $.post('{{ url("product/attribute/delete") }}', {
                id: id,
                _token: '{{ csrf_token() }}'
            }, function(data) {
    ...
    function add_option() {
        var html =
            '<div class="option_single"> <hr/> <div class="form-group"> <button class="btn btn-danger" onclick="$(this).parent().parent().remove();"><span class="fa fa-trash"></span></button> </div> <div class="form-group"> <label for="option_names">Option Name</label> <input type="text" id="option_names" name="option_names[]" class="form-control" required /> </div> </div>'
